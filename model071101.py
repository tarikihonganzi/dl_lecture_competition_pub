...
class VQAModel(nn.Module):
    def __init__(self, vocab_size: int, n_answer: int):
        ...
        self.text_dropoout = nn.Dropout(0.2)
        # batch_first=Trueが大事！
        ...
        self.fc4 = nn.Linear(512, n_answer)
    ...
    def forward(self, image, question):
        ...
        embeds_2 = self.fc_text(embeds)
        embeds = F.leaky_relu(embeds + embeds_2)
        embeds = self.text_dropoout(embeds)
        ...
        # question_feature = F.leaky_relu(torch.squeeze((tag_space + lstm_out[0]))
...
for epoch in range(num_epoch):
    ...
    if test_acc > best_test_acc:
      ...
      best_epoch = epoch
    torch.save(model.state_dict(), f"./model_data/071101/model_{epoch}.pth")
...
# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071101/model_{best_epoch}.pth"))
...
torch.save(model.state_dict(), "model_071101_best.pth")
np.save("submission_071003_best.npy", submission)

# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071101/model_{num_epoch - 1}.pth"))
...
submission = [train_dataset.idx2answer[id] for id in submission]
submission = np.array(submission)
torch.save(model.state_dict(), "model_071101_normal.pth")
np.save("submission_071101_normal.npy", submission)
