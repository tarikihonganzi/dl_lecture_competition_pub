...
for epoch in range(num_epoch):
    ...
    if test_acc > best_test_acc:
      ...
      best_epoch = epoch
    torch.save(model.state_dict(), f"./model_data/071102/model_{epoch}.pth")
...
# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071102/model_{best_epoch}.pth"))
...
torch.save(model.state_dict(), "model_071102_best.pth")
np.save("submission_071102_best.npy", submission)

# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071102/model_{num_epoch - 1}.pth"))
...
submission = [train_dataset.idx2answer[id] for id in submission]
submission = np.array(submission)
torch.save(model.state_dict(), "model_071102_normal.pth")
np.save("submission_071102_normal.npy", submission)
