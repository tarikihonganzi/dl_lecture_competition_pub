...
train_dataset = VQADataset(df_path="./data/trainv8.json", image_dir="./data/train", transform=transform_train, transform_adv=transform_train_adv, answer=True)

valid_dataset = VQADataset(df_path="./data/testv4.json", image_dir="./data/train", transform=transform_test, transform_adv=transform_test, answer=True)
...
for epoch in range(num_epoch):
    ...
    if test_acc > best_test_acc:
      ...
      best_epoch = epoch
    torch.save(model.state_dict(), f"./model_data/071001/model_{epoch}.pth")
...
# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071001/model_{best_epoch}.pth"))
...
torch.save(model.state_dict(), "model_071001_best.pth")
np.save("submission_071001_best.npy", submission)

# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071001/model_{num_epoch - 1}.pth"))
...
submission = [train_dataset.idx2answer[id] for id in submission]
submission = np.array(submission)
torch.save(model.state_dict(), "model_071001_normal.pth")
np.save("submission_071001_normal.npy", submission)
