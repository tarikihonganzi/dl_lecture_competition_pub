...
for epoch in range(num_epoch):
    ...
    torch.save(model.state_dict(), f"./model_data/071002/model_{epoch}.pth")
...
# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071002/model_{best_epoch}.pth"))
...
torch.save(model.state_dict(), "model_071002_best.pth")
np.save("submission_071002_best.npy", submission)

# modelの読み込み
model.load_state_dict(torch.load(f"./model_data/071002/model_{num_epoch - 1}.pth"))
...
submission = [train_dataset.idx2answer[id] for id in submission]
submission = np.array(submission)
torch.save(model.state_dict(), "model_071002_normal.pth")
np.save("submission_071002_normal.npy", submission)
